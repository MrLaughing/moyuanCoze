...
class WeekOverviewView @JvmOverloads constructor(
    ...
    // 墨色配色
    private val inkDark = ContextCompat.getColor(context, R.color.accent_green)
    private val inkLight = ContextCompat.getColor(context, R.color.border_light)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0 until barCount) {
            ...
            if (recordDate == LocalDate.now()) {
                barPaint.color = quietGold
                canvas.drawCircle(left + barWidth / 2, h - 1.5f * density, 1.5f * density, barPaint)
            }
        }
    }
}
